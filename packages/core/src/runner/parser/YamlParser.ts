...
export class YamlParser {
     ...
     */
    constructor(schemaPath: string, fs: IFileSystem) {
    ...
    async loadSchema(): Promise<void> {
        ...
        try {
            const schemaContent = await this.fs.readTextFile(this.schemaPath);
    ...
    async parse(filePath: string): Promise<RadiusRequest> {
        ...
        let content: string;
        try {
            content = await this.fs.readTextFile(filePath);
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unknown error';
    ...
    static isRequestFile(filePath: string): boolean {
        return filePath.toLowerCase().endsWith('.rd');
    }
}
