...
import type { IHttpClient } from '../types/http.js';
...
/**
 * A standard key-value entry with metadata.
...
/**
 * Form data specific entry (can be file or text).
 */
export interface FormDataEntry extends KeyValueEntry {
    type?: 'text' | 'file';
}

// =============================================================================
// RadiusRequest - Parsed from .rd files (V2 Schema)
// =============================================================================
...
/**
 * Request body configuration (V2).
 */
export interface RequestBody {
    /** High-level explicit body type matching GUI selector */
    type: 'json' | 'form-data' | 'urlencoded' | 'binary' | 'graphql' | 'text' | 'xml' | 'html' | 'none';

    /** Raw text content (for json, xml, html, text, graphql raw) */
    text?: string;

    /** Key-Value content (for form-data, urlencoded) */
    form?: FormDataEntry[];
...
export interface RequestDefinition {
    ...
    url: string;
    /** URL query parameters (Array to support duplicates/ordering/state) */
    params?: KeyValueEntry[];
    /** HTTP headers (Array to support duplicates/ordering/state) */
...
export interface RadiusRequest {
    ...
    /** Authentication configuration (can be at top level or inside request in V2, usually normalized) */
    auth?: AuthConfig;
    /** Pre/post scripts */
